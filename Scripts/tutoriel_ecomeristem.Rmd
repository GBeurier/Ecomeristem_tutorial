...
Tout d'abord, installez le package recomeristem, disponible sur github 
à l'adresse suivante :

https://github.com/GBeurier/Ecomeristem_tutorial

Vérifiez bien la version de R installé sur votre machine afin de récuperer
la bonne version du package recomeristem.
...
Si tout s'est bien déroulé, le package recomeristem peut être chargé de la façon 
suivante :
...
Le package recomeristem est un interfaçage entre le langage de programmation R 
et le modèle Ecomeristem à travers des fonctions de base permettant de 
communiqueravec le modèle (et des simulations).

## 1. Données d'entrée

La première étape consiste à importer les données d'entrée en utilisant les 
fonctions suivantes du package recomeristem :
...
Le seconde étape consiste à initialiser une simulation. Cette opération va
...
paramètres). L'initialisation dans recomeristem se fait de la façon suivante :
...
précédemment importés (paramètres, météos et observations) ainsi qu'un 
nom permettant d'identifier une simulation (plusieurs simulations peuvent être
initiés en même temps). 
...
objets déjà créés et les informations utiles à la simulation sont modifiés à la 
volée).
...
La particularité de ce dataframe est qu'il ne contient que les valeurs simulés
aux dates auxquels il y a une observations (et pour les traits phénotypiques 
observés).
...
correspondant aux dates et traits du dataframe d'observations 
...
ne sont pas présents dans le dataframe de résultats de simulation, il est aussi
...
Dans ce tutoriel on va voir l'utilisation du package recomeristem pour 
...
paramètres à tester (valeurs entre les bornes définit dans "bounds",
pour les paramètres définit dans "paramNames")
...
les travaux récents, plutôt autour de 2500-5000 itérations).
...
paramètres présenté dans ce tutoriel : l'erreur est calculé sur un ensemble
...
personnes intéressés, voir par exemple :
...
pour mettre de poids sur l'une ou l'autre variable prioritaire par exemple.
